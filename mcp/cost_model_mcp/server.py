...
"""OpenCode MCP server for cost model mcp."""
...
def main() -> int:
    tools = {
        "append_record": (
            schema(
                "append_record",
                "Append a JSONL cost record",
                {
                    "path": string_property("JSONL file path", None),
                    "record": {"type": "object", "description": "Cost record object"},
                },
                ["path", "record"],
            ),
            append_record,
    ...
    return SimpleMcpServer("cost_model_mcp", tools).serve()
